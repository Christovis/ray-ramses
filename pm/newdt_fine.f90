subroutine newdt_fine(ilevel)
  ...
  if(poisson.and.gravity_type<=0)then
     ...
     if(rho_max(ilevel).gt.0.0D0) then
        tff=sqrt(threepi2/8./fourpi/ rho_max(ilevel)        )
     else
        tff=sqrt(threepi2/8./fourpi/(rho_max(ilevel)+1.0d-6))
     end if
  ...
  if(pic) then
     ...
     ! FOR TESTING RAYS!!!
     !write(*,*) myid, "REDUCING TIME STEP (bla)!!!"
     !dtnew(ilevel) = 7.179E-03*1.0D-2
     !dtnew(ilevel) = dtnew(ilevel)*1.0D-2
     !dt_loc = dt_loc*1.0e-2  ! in first paper
     !dt_loc = dt_loc*1.0e-1
