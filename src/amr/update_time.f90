subroutine update_time(ilevel)
  ...
  ! We save the scale factor here (before it is updated below)
  ! to use in the calculation of the maximum distance a ray can travel
  ! in a time step in ray/ray_step.f90
  ! Start: Sownak - ISW (28/10/2015)
  !if(ray) then
  !   aexp_old_ray(ilevel) = aexp
  !end if
  ! End: Sownak
  ...
  if(nstep_coarse .ne. nstep_coarse_old)then
     ...
     if(t>=tout(noutput).or.aexp>=aout(noutput).or. &
        ...
        if(.not.ray) then
           ...
           ray_end_of_sim = .true.
           if(ray_all_finished) call clean_stop
        end if
     end if
