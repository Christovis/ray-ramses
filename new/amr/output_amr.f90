...
subroutine dump_all
  ...
  ifout=ifout+1
  !if(t>=tout(iout).or.aexp>=aout(iout))iout=iout+1       ! Alex: for_exact_output_times
  if(t>=tout(iout).or.aexp>=aout(iout)-1.0D-8)iout=iout+1 ! Alex: for_exact_output_times
  output_done=.true.
