...
subroutine ray_integrate_ngp(iray,ilevel,ind,chi_A,ray_fields_c)
  ...
  if(ray_do_kappa) then 
     ...
     integral_cell(i_ray_kappa:i_ray_kappa+1)   = -(0.5D0*(chi_B**2-chi_A**2)/2.0D0-(chi_B**3-chi_A**3)/(3.0D0*ray_chi_s)) * &
...
!                                ( 0.0D0                         )*ray_fields_c(i_ray_tidal+2) + & !zz term
     ...
     integral_cell(i_ray_shear:i_ray_shear+1)   = -(0.5D0*(chi_B**2-chi_A**2)/2.0D0-(chi_B**3-chi_A**3)/(3.0D0*ray_chi_s)) * &
  ...
  if(ray_do_deflt) then
     ...
     integral_cell(i_ray_deflt:i_ray_deflt+1)   = -(0.5D0*(chi_B**2-chi_A**2)/2.0D0-(chi_B**3-chi_A**3)/(3.0D0*ray_chi_s)) * &
  ...
  if(ray_do_iswrs) then
     ...
     aexp_mean              = (aexp_new_ray(ilevel)+aexp_old_ray(ilevel))/2.0D0
     ray_quants(i_ray_iswrs) = ray_quants(i_ray_iswrs)+integral_cell(i_ray_iswrs)*aexp_mean*ray_Lbox**3*100.0D0**3
  endif
  ! +++ add new stuff here to calculate other observables +++
