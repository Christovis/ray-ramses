...
subroutine refine_fine(ilevel)
  ...
  ! Lensing stuff
  !--------------------------
  if(ray.and.ray_initialized) ex_father = father
     ...
     do igrid=1,ncache,nvector  ! Loop over grids
        ...
        do ind=1,twotondim
           ...
           if(ncreate_tmp>0)then
              ...
              call make_grid_fine(ind_grid_tmp,ind_cell_tmp,ind,ilevel+1,ncreate_tmp,ibound,boundary_region)
           end if
        end do
     end do
  end do
  ...
  ! Lensing stuff
  !------------------------
  if(ray.and.ray_initialized) call check_ray_grid_refinement(ilevel)
