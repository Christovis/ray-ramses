...
subroutine ray_fields_in_corners(ilevel,igrid,ind,ray_fields)
  ...
  integer :: i
  integer :: icell,old_size,new_size                               ! BAOJIU-04-10
  real(dp),allocatable,dimension(:,:)   :: temp_arr1               ! BAOJIU-04-10
  real(dp),allocatable,dimension(:,:,:) :: temp_arr2               ! BAOJIU-04-10
  logical :: increase_array                                        ! BAOJIU-04-10
        ...
        if(igrid_nbor.eq.0) then
           ...
           if(son(icell_nbor).eq.0) then
              ...
              if(ray_kappa_method.ne.2) then                       ! BAOJIU-29-09
                 do i=1,3
                    ray_fields(i+7,itarget) = ray_fields(i+7,itarget)+f(icell_nbor,i)
                 enddo
              end if                                               ! BAOJIU-29-09
              ray_fields(11,itarget) = ray_fields(11,itarget)+phi(icell_nbor)
        ...
        do ind_nbor=1,8
           ...
           if(son(icell_nbor).eq.0) then                           ! neighbouring level-ilevel cell unrefined
              ...
              if(ray_kappa_method.ne.1) then                       ! BAOJIU-29-09
                 call tidal_tensor(ilevel,igrid_nbor,icell_nbor,ray_lll_mmm(itarget,ind_nbor,ind),tidal)
                 do i=1,6
                    ray_fields(i+1,itarget) = ray_fields(i+1,itarget)+weight*tidal(i)
                 enddo
              end if                                               ! BAOJIU-29-09
              if(ray_kappa_method.ne.2) then                       ! BAOJIU-29-09
                 do i=1,3
                    ray_fields(i+7,itarget) = ray_fields(i+7,itarget)+weight*f(icell_nbor,i)
                 enddo
              end if                                               ! BAOJIU-29-09
              ray_fields(11,itarget) = ray_fields(11,itarget)+weight*phi(icell_nbor)
              ...
              if(ray_do_isw) then
                 ...
                 !phidot = analytic_isw(ilevel, igrid, ind, itarget, aexp_mean, zexp_mean)
                 !phidot                 = (phi(icell_nbor)/aexp_new_ray(ilevel)**2-phi_old(icell_nbor)/aexp_old_ray(ilevel)**2)
                 ray_fields(12,itarget) = ray_fields(12,itarget)+weight*phidot
              end if
              ! End: Sownak
              ...
              if(ray_kappa_method.ne.2) then                       ! BAOJIU-29-09
                 do i=1,3
                    ray_fields(i+7,itarget) = ray_fields(i+7,itarget)+weight*f(icell_nbor_fine,i)
                 enddo
              end if                                               ! BAOJIU-29-09
              ray_fields(11,itarget) = ray_fields(11,itarget)+weight*phi(icell_nbor_fine)
  ...
  if(all(normal_vertice)) then                                     ! if not at refinement boundary, calculation finishes here
     ...
     if(ray_ncells.gt.size(ray_stored_dens,1)) then

        increase_array = .true.

        old_size = size(ray_stored_dens,1)
        !new_size = old_size+old_size/2
        new_size = 5.0*old_size

        write(*,*) 'ray_fields_in_corners: enlarging corner arrays',old_size,new_size 

        allocate(temp_arr1(1:old_size,1:8))

        temp_arr1 = ray_stored_dens
        deallocate(ray_stored_dens)
        allocate(ray_stored_dens(1:new_size,1:8))
        ray_stored_dens = 0.0D0
        ray_stored_dens(1:old_size,:) = temp_arr1(1:old_size,:)

        temp_arr1 = ray_stored_pots
        deallocate(ray_stored_pots)
        allocate(ray_stored_pots(1:new_size,1:8))
        ray_stored_pots = 0.0D0
        ray_stored_pots(1:old_size,:) = temp_arr1(1:old_size,:)

        ! Start: Sownak - ISW (26/10/2015)
        if (ray_do_isw) then
           temp_arr1 = ray_stored_phidot
           deallocate(ray_stored_phidot)
           allocate(ray_stored_phidot(1:new_size,1:8))
           ray_stored_phidot = 0.0D0
           ray_stored_phidot(1:old_size,:) = temp_arr1(1:old_size,:)
        end if
        ! End: Sownak

        deallocate(temp_arr1)

     end if

     ray_stored_dens(ray_ncells,1:8) = ray_fields(1, 1:8)
     ray_stored_pots(ray_ncells,1:8) = ray_fields(11,1:8)
     ! Start: Sownak (26/10/2015)
     if (ray_do_isw) then
        ray_stored_phidot(ray_ncells,1:8) = ray_fields(12,1:8)
     end if
     ! End: Sownak

     if(ray_do_kappa.and.ray_kappa_method.ne.2) then

        if(increase_array) then

           allocate(temp_arr2(1:old_size,1:3,1:8))

           temp_arr2 = ray_stored_force
           deallocate(ray_stored_force)
           allocate(ray_stored_force(1:new_size,1:3,1:8))
           ray_stored_force = 0.0D0
           ray_stored_force(1:old_size,:,:) = temp_arr2(1:old_size,:,:)

           deallocate(temp_arr2)

        end if

        ray_stored_force(ray_ncells,1:3,1:8) = ray_fields(8:10,1:8)
     ...
     if(ray_do_shear.or.(ray_do_kappa.and.ray_kappa_method.ne.1)) then
        ...
        if(increase_array) then
           ...
           allocate(temp_arr2(1:old_size,1:6,1:8))

           temp_arr2 = ray_stored_tidal
           deallocate(ray_stored_tidal)
           allocate(ray_stored_tidal(1:new_size,1:6,1:8))
           ray_stored_tidal = 0.0D0
           ray_stored_tidal(1:old_size,:,:) = temp_arr2(1:old_size,:,:)

           deallocate(temp_arr2)

        end if

        ray_stored_tidal(ray_ncells,1:6,1:8) = ray_fields(2:7,1:8)
  ...
  do jtarget=1,8
     ...
     do ind_nbor=1,8
        ...
        if(igrid_nbor.eq.0) then                                   ! neighbouring level-ilevel grid not exist
           ...
           weight     = 1.0D0
           ! Claudio:  grid that corresponds to this cell_nbor
           ...
           down_level = .true.                                     ! BAOJIU-11-02-2016
           ...
           down_level = .false.                                    ! BAOJIU-11-02-2016
        ...
        if(ray_kappa_method.ne.1) then                             ! BAOJIU-29-09
           do i=1,6
              ray_fields_tmp(i+1,jtarget) = ray_fields_tmp(i+1,jtarget)+weight*tidal(i)
           enddo
        end if                                                     ! BAOJIU-29-09
        if(ray_kappa_method.ne.2) then                             ! BAOJIU-29-09
           do i=1,3
              ray_fields_tmp(i+7,jtarget) = ray_fields_tmp(i+7,jtarget)+weight*f(icell_nbor,i)
           enddo
        end if                                                     ! BAOJIU-29-09
        ray_fields_tmp(11,jtarget) = ray_fields_tmp(11,jtarget)+weight*phi(icell_nbor)
        ! Start: Sownak - ISW (26/10/2015)
        if(ray_do_isw) then                                        ! BAOJIU-11-02-2016
           if(down_level) then
           ...
           ray_fields_tmp(12,jtarget) = ray_fields_tmp(12,jtarget) + weight*phidot
        end if                                                     ! BAOJIU-11-02-2016
        ! End: Sownak
     end do
...
! -----------------------------------------------------------
! ray_interpolate (THIS COMES FROM ALEX'S VERSION 09.08.2015)
! -----------------------------------------------------------
        ...
        if(igrid_nbor .eq. 0) then
           ...
           do ind_father = 1, 27
              ...
              if(ray_kappa_method.ne.1) then                       ! BAOJIU-29-09
                 ...
                 do i=1,6
                    ray_fields(i+1,itarget) = ray_fields(i+1,itarget)+dble(weight)*tidal(i)
                 enddo
              end if                                               ! BAOJIU-29-09
              if(ray_kappa_method.ne.2) then                       ! BAOJIU-29-09
                 do i=1,3
                    ray_fields(i+7,itarget) = ray_fields(i+7,itarget)+dble(weight)*f(indice,i)
                 enddo
              end if                                               ! BAOJIU-29-09
              ray_fields(11,itarget) = ray_fields(11,itarget)+dble(weight)*phi(indice)
